      ...
      {items.map((nutrition, index) => (
        <div key={`${nutrition.nutrient}-${index}`} className="mini-summary-card">
          ...
          <div className="mini-summary-fields">
            <input type="number" value={nutrition.value} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onValueChange(index, e.target.value)} placeholder="Amount" />
            <select value={nutrition.unit} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onUnitChange(index, e.target.value)}>{unitOptions.map((u) => <option key={u} value={u}>{u}</option>)}</select>
          </div>
        </div>
      ))}
    </div>
          ...
          <div key={`recipe-ingredient-${index}`} className="mini-summary-card ingredient-summary-card">
            ...
            <div className="mini-summary-fields ingredient-summary-fields">
              <div className="ingredient-amount-row">
                <input type="number" value={item.quantity} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onChange(index, { quantity: Number(e.target.value) })} placeholder="Amt" />
                <select value={item.unit} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onChange(index, { unit: e.target.value })}>{unitOptions.map((u) => <option key={u} value={u}>{u}</option>)}</select>
              </div>
              <input value={item.note || ''} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onChange(index, { note: e.target.value })} placeholder="Note" />
              <select value={item.ingredientId} onChange={(e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => onChange(index, { ingredientId: Number(e.target.value), ingredientName: ingredients.find((ing) => String(getItemId(ing)) === String(e.target.value))?.name || '' })}>{ingredients.map((ing) => <option key={getItemId(ing)} value={getItemId(ing)}>{ing.name}</option>)}</select>
            </div>
          </div>
        );
      })}
